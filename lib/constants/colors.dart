import 'package:flutter/material.dart';

class AppColor {
  static Color transactionBtn_color = Color.fromARGB(255, 241, 241, 241);
  static Color mainBtn_color = const Color(0xFF0095BE);
  static Color mainBtn_color2 = const Color(0xFF00A3D0);
  static Color secondary_color = const Color(0xFFBFF1FF);
  static Color subtitle = const Color(0xFF919191);
  static Color eti = const Color(0xFFDEFFF6);
  static Color red = Color.fromARGB(255, 255, 60, 60);
  static Color blues = const Color(0xFF3D1EFF);
  static Color lightgray = const Color(0xFFF0F0F0);
  static Color arrowcolor = Color.fromARGB(255, 195, 195, 195);
  static Color black = Color.fromARGB(255, 0, 0, 0);
  static Color white = Color.fromARGB(255, 255, 255, 255);
  static Color primaryDeleteButton_color = const Color(0xFFFFE7E7);
  static Color secondaryDeleteButton_color = const Color(0xFFF8F8F8);
}
