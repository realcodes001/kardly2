...
class _MastercardCardState extends State<MastercardCard>
  ...
  void _flipCard() {
    if (_showBack) {
      ...
      // Timer to stop the shimmer effect after 5 seconds
  ...
  Widget _buildBackCard() {
    return Container(
      ...
      child: Row(
        children: [
          // Using RepaintBoundary to isolate sections that shouldn't be rebuilt unnecessarily
          RepaintBoundary(
            child: Container(
  ...
  Widget _buildShimmerBlock({required double width, required double height}) {
    // Reduced shimmer size to optimize
  ...
  // Method to build the normal static text (non-shimmer state)
